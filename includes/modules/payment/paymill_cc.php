...
class paymill_cc extends paymill_abstract
{
    ...
    function pre_confirmation_check()
    {
        ...
        for ($i=1; $i<13; $i++) {
            $months_array[$i] = array(zen_output_string(sprintf('%02d', $i)),
                                      zen_output_string_protected(strftime('%B',mktime(0,0,0,$i,1,2000))));
        }
        ...
        for ($i=$today['year']; $i < $today['year']+10; $i++) {
            $years_array[$i] = array(zen_output_string(strftime('%Y',mktime(0,0,0,1,1,$i))),
                                     zen_output_string_protected(strftime('%Y',mktime(0,0,0,1,1,$i))));
        } 
        ...
        $oscTemplate->addBlock('<form id="paymill_form" action="' . zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL') . '" method="post" style="display: none;"></form>', 'footer_scripts');
    }
    ...
    function check()
    {
        if (!isset($this->_check)) {
            $check_query = zen_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key = 'MODULE_PAYMENT_PAYMILL_CC_STATUS'");
    ...
    function install()
    {
        ...
        zen_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_description, configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . MODULE_PAYMENT_PAYMILL_CC_LABEL_TITLE . "', '" . MODULE_PAYMENT_PAYMILL_CC_LABEL_DESC . "', 'MODULE_PAYMENT_PAYMILL_CC_LABEL', 'False', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
