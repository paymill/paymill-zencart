...
?>
      ...
      <tr>
        ...
        <td colspan="2" align="right" >
            ...
            <?php 
            $transaction = $db->Execute("SELECT * FROM " . DB_PREFIX . "pi_paymill_transaction WHERE order_id = '" . $_GET['oID'] . "'");
            if (isset($transaction->fields['transaction_id']) && isset($transaction->fields['transaction_id'])) {
                include(dirname(__FILE__) . '/../includes/languages/' . $_SESSION['language'] . '/modules/payment/' . $transaction->fields['payment_code'] . '.php');
                echo '<a href="' . zen_href_link('paymill_refund.php', 'oID=' . $_GET['oID']) . '"><input type="button" value="' . PAYMILL_REFUND_BUTTON_TEXT . '"/></a>';
            }
            ?>
            </div>
        </td>
